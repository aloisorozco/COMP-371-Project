...
void main()
{
    ...
    vec4 baseColor;
    if (useTexture) {
       baseColor = texture(textureSampler, vTextCoord) ;
       baseColor = vec4(baseColor.rgb, 0.4) + vec4(worldColor, 1.0)* (0.6);
    } 
    else {
        baseColor = vec4(worldColor, 1.0);
    }
    ...
    vec3 color = mix(baseColor.rgb, worldColor, float(useTexture));
    
    color = (specular + diffuse + ambient) * baseColor.rgb;
